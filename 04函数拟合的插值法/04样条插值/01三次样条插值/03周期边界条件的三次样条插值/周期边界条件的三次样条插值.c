...
double cubicSpline(long n, struct Point *points, double xs, double *h, long location){
    ...
    // 这里自定义边界条件 一阶导与二阶导的值
    printf("请输入区间端点导数值 \n");
    printf("左端点的二阶导数值 f'(%lf)：", points[0].x);
    double ddy_left;
    scanf("%lf", &ddy_left);
    printf("右端点的二阶导数值 f'(%lf)：", points[n-1].x);
    double ddy_right;
    scanf("%lf", &ddy_right);

    // 第一行求解 M0 其实好像并没有什么好的初值，但是因为 M0 = M_{n-1} 
    // 所以最后只要把 M_{n-1}的结果放到 M_0 里面就行
    ...
    // 对最后一行单独赋值
    alpha[n-1] = h[n-2] / (h[0] + h[n-2]);
    b[n-1]=2.0;
    c[n-1] = (6/(h[0] + h[n-2])) * ((points[1].y - points[0].y) / h[0] - (points[n-1].y - points[n-2].y) / h[n-2]);
    M[n-1] = ddy_right;
